...
def replace_linear_layers(model, old_class=nn.Linear, new_class=SwitchBackLinear):
    for name, module in model.named_children():
        # Check if the module is an instance of the old_class
        if isinstance(module, old_class):
            # Skip replacing k_proj and v_proj layers
            if name in ['k_proj', 'v_proj']:
                continue  # Skip these layers
            
            # Replace Linear layer with SwitchBackLinear, preserving in/out features and bias
            new_module = new_class(module.in_features, module.out_features, bias=module.bias is not None)
            
            # Copy weights and bias from the old module to the new module
            ...
            replace_linear_layers(module, old_class, new_class)
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    do_eval: bool = field(default=False, metadata={"help": 'To train or not to train, that is the question?'})
...
def get_accelerate_model(args, checkpoint_dir, use_swichback):
    ...
    if((use_swichback is not None) and (use_swichback.lower() == 'true' or use_swichback == '1')):
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    if args.do_train:
        ...
        train_complete_size = (len(train_dataset)//train_batch_size)*train_batch_size
        train_dataset = train_dataset.select(range(train_complete_size))
...
def train():
    ...
    model, tokenizer = get_accelerate_model(args, checkpoint_dir, extra_args[1])

    model.config.use_cache = False
    
    if((extra_args[1] is not None) and (extra_args[1].lower() == 'true' or extra_args[1] == '1')):
...
if __name__ == "__main__":
    ...
    seconds = end - start
    minutes = seconds / 60
    print(f"END FINE-TUNE TIME SPENT: {int(seconds)} seconds")
    print(f"END FINE-TUNE TIME SPENT: {minutes:.2f} minutes")
